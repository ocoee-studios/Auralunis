  ...
  return (
    <G>
      ...
      {showLabels && (() => {
        ...
        const placed = placeLabel ? placeLabel(fallbackX, cy + 4, "Moon", 19, avoid) : null;
        // PRIORITY 1, with the planets. Never dropped.
        const lp = placed && Number.isFinite(placed.x) ? placed : { x: fallbackX, y: cy + 4 };
        return (
          <G>
            <SvgText x={lp.x} y={lp.y} fill="none" stroke="#050914" strokeWidth={3.4} strokeOpacity={0.55} fontSize={19} fontWeight="700">
              Moon
            </SvgText>
            <SvgText x={lp.x} y={lp.y} fill={palette.starLabel} fontSize={19} fontWeight="700" opacity={0.98}>
              Moon
            </SvgText>
          </G>
        );
